...
- mock 回退：真实数据不可用时仍保留可浏览界面。
- 元数据缓存：英雄/物品目录在 client 层缓存，避免重复构建。
...
- 队友协同页。
- GPM/XPM 趋势与活跃时段统计。
- peers + recent matches 结合推断最近遇到时间。
...
- OpenDota 限流或抖动：保留明确错误提示，并允许页面留在 mock/空态。

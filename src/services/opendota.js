...
const fetchJson = async (path, signal) => {
  ...
  if (!response.ok) {
    if (response.status === 404) {
      throw new Error('未找到该玩家，请检查 ID 是否正确。');
    }
    if (response.status === 429) {
      throw new Error('OpenDota 请求过于频繁，请稍后重试。');
    }
    throw new Error(`OpenDota 请求失败（HTTP ${response.status}）。`);
  }

  return response.json();
};

const getHeroesMap = async (signal) => {
  if (heroesCache) {
    return heroesCache;
  }

  const heroes = await fetchJson('/heroes', signal);
...
const buildHeroPerformance = (matches, heroesMap) => {
  ...
  matches.forEach((match) => {
    ...
    const role = laneRoleMap[match.lane_role] ?? (match.is_roaming ? '游走' : '未标注');
  ...
  return Array.from(aggregate.values())
    .map((record) => {
      ...
      return {
        hero: record.hero,
        role: getMainRole(record.roleCount),
...
const buildRankDistribution = (matches) => {
...
export const fetchPlayerWindowAnalytics = async (accountId, days, signal) => {
  const [player, matches, heroesMap] = await Promise.all([
    fetchJson(`/players/${accountId}`, signal),
    fetchJson(`/players/${accountId}/matches?date=${days}`, signal),
    getHeroesMap(signal),
  ]);

  const validMatches = Array.isArray(matches) ? matches.filter((item) => item.start_time) : [];
  if (validMatches.length === 0) {
    throw new Error(`该玩家最近 ${days} 天没有公开对局数据。`);
  }

  const heroPerformance = buildHeroPerformance(validMatches, heroesMap);
  const dailyWinRate = buildDailyWinRate(validMatches, days);
  const rankDistribution = buildRankDistribution(validMatches);

  return {
    playerName: player?.profile?.personaname ?? `玩家 ${accountId}`,
    heroPerformance,
    dailyWinRate,
    rankDistribution,
